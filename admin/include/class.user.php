...
public function register_admin($myarray) {
        ...
        $profile_img = $myarray['profile_img'];
          try {
            ...
            $stmt = $this->_dbh->prepare("UPDATE users SET fname=:fname,lname=:lname,"
                    ...
                    . "website=:website,profile_img=:profile_img WHERE id=$user_id");
...
//        print_r($data);        exit();
    ...
    public function get_all_random_user() {
        ...
        $user_id = $_SESSION['user_session'];
        $stmt = $this->_dbh->prepare("SELECT * FROM users order by RAND() ASC");
        $stmt->execute(array(":user_id" => $user_id));
        $userRow = $stmt->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function profile_username() {
        $profileid = $_GET['id'];
        $user_id = $_SESSION['user_session'];
        $stmt = $this->_dbh->prepare("SELECT * FROM users WHERE id=$profileid");
        $stmt->execute();
        $userRow = $stmt->fetch(PDO::FETCH_ASSOC);
...
//    }
    public function get_all_user_profile() {
        ...
        //echo $profile_id;
        $user_id = $_SESSION['user_session'];
        $stmt = $this->_dbh->prepare("SELECT * FROM users WHERE id=:profile_id order by id DESC");
...
public function getvalue_comments() {
        ...
        $user_id = $_SESSION['user_session'];
        $stmt = $this->_dbh->prepare("SELECT *,comments.id as commentid FROM comments INNER JOIN users ON comments.user_id=users.id where comments.user_id=:user_id && comments.profileid='0' or comments.profileid=:user_id order by comments.id DESC");
        $stmt->execute(array(":user_id" => $user_id));
        $userRow = $stmt->fetchAll(PDO::FETCH_ASSOC);
  ...
  public function getvalue_userprofile_comments() {
       ...
       // echo $profile_id;
        $user_id = $_SESSION['user_session'];
        $stmt = $this->_dbh->prepare("SELECT *,comments.id as commentid FROM comments INNER JOIN users ON comments.user_id=users.id where  comments.user_id=:profile_id && comments.profileid='0' or comments.profileid=:profile_id order by comments.id DESC");
        $stmt->execute(array(":profile_id" => $profile_id));
        $userRow = $stmt->fetchAll(PDO::FETCH_ASSOC);
