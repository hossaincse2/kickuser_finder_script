       ...
       // print_r($myarray);
        ...
        try {
         //   $new_password = password_hash($user_pass, PASSWORD_DEFAULT);
            ...
            $stmt = $this->_dbh->prepare("INSERT INTO blog(title,description,metakey,metadescription,date) VALUES(:title,:description,:metakey,:metadescription,:date)");
...
//        print_r($data);        exit();
  ...
  $id=$_GET['id'];
$result = $this->_dbh->prepare("DELETE FROM users WHERE username= :id");
$result->bindParam(':id', $id);
$result->execute();
      }
    ...
    public function get_reports(){
        $stmt = $this->_dbh->prepare("SELECT au.id as rid,pu.id as uid,au.kikuser as rkik,pu.kikuser as ukik,au.date,report.report, au.suspended as suspended FROM report INNER JOIN users au ON report.report_id=au.username Join users pu on report.user_id = pu.username order by report.id desc LIMIT 0, 25");
        $stmt->execute();
        $userRow = $stmt->fetchAll(PDO::FETCH_ASSOC);
